...
const PreWeddingGen: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Collect references
      const references = couplePhotos.map((p) => ({
        base64: p.base64,
        mimeType: p.mimeType,
      }));
      if (dressPhoto) {
      ...
      const functionUrl =
        "https://us-central1-dreamapp-c833c.cloudfunctions.net/generatePreWeddingPhoto";
      ...
      const response = await fetch(functionUrl, {
        ...
        body: JSON.stringify({
          ...
          customPrompt: dressPhoto
            ? "Include the style of the uploaded dress."
            : undefined,
        }),
      });
    ...
    <div className="max-w-6xl mx-auto pb-12">
      <div className="text-center mb-10 space-y-3">
        ...
        <p className="text-slate-400 max-w-2xl mx-auto">
          Upload your photos and let our AI transform them into cinematic
          masterpieces.
        </p>
      </div>
          ...
          <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-xl">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2">
              {couplePhotos.map((photo, index) => (
                <div
                  key={index}
                  className="relative shrink-0 w-24 h-24 rounded-lg overflow-hidden group border border-slate-600"
                >
                  <img
                    src={photo.preview}
            ...
            {dressPhoto ? (
              ...
              <label className="flex items-center justify-center w-full h-20 border-2 border-dashed border-slate-600 rounded-lg cursor-pointer hover:border-rose-500 hover:bg-slate-700/50 transition-all">
                <span className="text-sm text-slate-400">
                  Click to upload dress/suit photo
                </span>
                <input
                  type="file"
                  className="hidden"
            ...
            <div className="flex-1 bg-slate-900 rounded-2xl border border-slate-700 flex items-center justify-center overflow-hidden relative">
              {isGenerating ? (
                <div className="text-center space-y-6 max-w-sm px-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-slate-500 text-sm">
                      Gemini is analyzing features, applying lighting, and
                      rendering the scene...
                    </p>
                  </div>
                </div>
                ...
                <div className="text-center space-y-4 px-6 opacity-40">
                  ...
                  <p className="text-lg font-serif text-slate-500">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {history.map((img, idx) => (
              ...
              >
                ...
                />
                <div
                  className={`absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent transition-opacity ${
                    generatedImage === img
                      ? "opacity-0"
